...
public class WolfAI : MonoBehaviour
{
    ...
    void SearchForTarget()
    {
        ...
        for(int i = 1; i < 50; i++)
        {
            hits = Physics2D.OverlapCircleAll(transform.position, Mathf.Exp(i), bushMask); //Creates a circle from this position (Wolf position) using Mathf.Exp(i) as radius. Mathf.Exp(i) - Return i to the power of i. Radius gets larger with each loop. Search for game objects which is on bushMask layer
